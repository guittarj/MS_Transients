---
title: Transient and persistent species differ in germination rates, not establishment
  rates, across a grassland metacommunity.
...
```{r assemble data, include = FALSE, cache = FALSE}
...
**Figure 2.** Numbers of individuals recorded in this study, grouped by life stage and shaded by local species status. "Persistent" species occur as adults in three or more of our four annual vegetation surveys, whereas "transient" seeds do not. Percentages are of transient species out of total life stage.
...
**Figure 3.** Seed density and richness of transient species by site, in raw numbers and as percentages of total seeds/species at each site, plotted by mean summer temperature. Summer is defined as the three warmest months at each site. Shape color is consistent with Fig. 1 and reflects mean annual precipitation values, from light blue to dark blue, of approximately 660 mm, 1300 mm, 2000 mm, and 2900 mm. Linear regression lines are solid when significant (p < 0.05) and dashed when marginally significant (p < 0.1). See Table S3 for linear regression summary statistics.
...
j <- mod_coefs %>%
  ...
  transmute(
    ...
    `Null model` = germNull, 
    `Local climate` = germClim,
    `Local climate + Local sp. status (P/T)` = germS,
    `Local climate + Local sp. status (P/ST/C/Wa/U)` = germT,
    `Local climate + Local sp. status (P/SP/D/We/U)` = germP)

AICs <- c('$\\Delta$AIC', NA, sprintf("%.2f", c(0, AIC(germClim) - AIC(germNull), 
                                                   AIC(germS) - AIC(germNull),
                                                   AIC(germT) - AIC(germNull), 
                                                   AIC(germP) - AIC(germNull))))
...
kable(j, format = 'latex', escape = F, booktabs = T, linesep = "", align = c('l','r','r','r','r','r','r')) %>%
  ...
  footnote(threeparttable = TRUE, general = "Comparison of z-scores from different GLM models (columns) predicting numbers of emerged seedlings by species and site. Abbreviations in column headers refer to the categories used in the model term denoting local species status, potentially including the putative climate origins of each transient species at each site: P = persistent, T = transient, ST = similar temperature (i.e., a transient from a site with a similar temperature), SP = similar precipitation, C = cooler, Wa = warmer, D = drier, We = wetter, or U = unknown climate. Data include all recorded seeds and seedlings that could be identified to species. N equals the number of unique species-by-site combinations for each category/predictor. Asterisks denote significance (*: p < 0.05, **: p < 0.001). Dashes indicate predictors that were not included in a given model. Including model terms for transient/persistent species status and then putative temperature and precipitation origins progressively improved model performance, as reflected in the reduction of AIC values relative to the null model that uses only local seed numbers as a predictor.", general_title = "Table 1.") %>%
  landscape()
...
```{r seed sampling rarefaction, include = FALSE}

# create dataframe which includes seed rain, seed bank, and all seeds
rf <- xS %>% 
  filter(stage == 'seed') %>%
  bind_rows(x) %>%
  filter(stage %in% c('rain','bank','seed')) %>%
  group_by(stage, site, sp) %>%
  summarise(abun = ceiling(sum(abun))) %>%
  spread(sp, abun, fill = 0) %>%
  as.data.frame()

#add row names (necessary for rarecurve(), perform rarefaction, 
row.names(rf) <- paste0(rf$stage, rf$site)
rf$site <- rf$stage <- NULL
tmp <- rarecurve(rf); dev.off()
tmp <- lapply(tmp, function(x) data.frame(N = seq_along(x), rich = x))
names(tmp) <- row.names(rf)
tmp <- do.call('rbind', tmp)
tmp$site <- unlist(lapply(strsplit(as.character(row.names(tmp)), '.', fixed = TRUE), function(x) x[1]))
tmp$stage <- substr(tmp$site, 1, nchar(tmp$site) - 3)
tmp$site <- substr(tmp$site, nchar(tmp$site) - 2, nchar(tmp$site))

#add climate
tmp <- tmp %>%
  left_join(site.meta[, c('site','temp','precip','temp.level','precip.level')], by = 'site') %>%
  mutate(
    stage = factor(stage_labels(stage)),
    MST = factor(c(6,9,10.5)[match(temp.level, c(1:3))]),
    MAP = factor(c(650,1300,2000,2900)[match(precip.level, c(1:4))])) %>%
  group_by(site, stage) %>%
  mutate(Nmax = max(N), richmax = max(rich))

```

```{r seed sampling rarefaction plot, fig.height = 3.5}

tmp1 <- tmp %>%
  group_by(temp.level) %>% 
  mutate(temp = paste(signif(mean(temp), 2), "°C")) %>%
  ungroup() %>%
  group_by(precip.level) %>% 
  mutate(precip = paste(signif(mean(precip), 2), "mm/yr")) %>%
  ungroup() %>%
  mutate(
...
ggplot(tmp1, aes(x = N, y = rich, group = site)) +
    geom_line() +
    geom_point(aes(x = Nmax, y = richmax, shape = MST, fill = MAP), size = 3, data = distinct(tmp1, site, stage, .keep_all = TRUE)) +
    ...
    scale_shape_manual(values = c(25,21,24), 'Mean annual precipitation') +
    theme_bw() +
    theme(
      panel.grid.minor = element_blank(),
      panel.grid.major = element_blank(),
      ...
      legend.position = 'bottom') +
    guides(
      fill = guide_legend(title.position = 'top', title.hjust = 0.5, override.aes = list(shape = 21)),
      shape = guide_legend(title.position = 'top', title.hjust = 0.5))

```

**Figure S1.** Rarefaction curves for seed sampling. Coloring and symbols are consistent with Figure 1. 
...
**Figure S2.** Whittaker rank abundance plots plotted by site. MST = Mean summer temperature, defined as the three warmest months at each site; MAP = Mean annual precipitation.
...
j <- x %>%
  ...
  group_by(site, temp, Stage = stage, trait) %>%
  filter(length(unique(id)) == 2) %>%
  summarise(cm_diff = cm[id == 'Transient'] - cm[id == 'Persistent']) %>%
  filter(!is.na(cm_diff))

mods <- j %>%
  group_by(trait, Stage) %>%
  do(mod = lm(cm_diff ~ temp, data = .)) %>%
  mutate(pval = summary(mod)$coefficients[8])

j <- j %>%
  left_join(mods[, c('trait','Stage','pval')], by = c("trait", "Stage"))

ggplot(j, aes(x = temp, y = cm_diff, color = Stage, fill = Stage)) +
    geom_hline(yintercept = 0, lty = 3, color = 'dark grey') +
    geom_point() + 
    stat_smooth(method = 'lm', se = FALSE, data = filter(j, pval < 0.05)) +
    facet_wrap(~trait, scales = 'free', ncol = 2) +
    th +
    labs(x = expression('Mean summer temperature ('*degree*'C)'), y = 'Transient community mean - Persistent community mean')

```

**Figure S4.** Community mean trait values of locally transient species relative to locally persistent species, grouped by site and life stage and plotted by mean summer temperature. Community means are calculated as the average trait value of all species present in the community. Values above the grey dotted line (y = 0) denote sites where the trait values of transient individuals were greater on average than the trait values of persistent individuals, and vice versa. Lines are shown only when linear regression is significant (p < 0.05).

\pagebreak

```{r CMs by precip, fig.height = 8}

j <- x %>%
  left_join(trait.data, by = 'sp') %>%
  left_join(site.meta[, c('site','precip')], by = 'site') %>%
  rename(`Seed mass` = seed.mass, 
         SLA = sla, 
         `Leaf area` = leaf.area, 
         `Maximum height` = max.height,
  ...
  mutate(stage = stage_labels(stage)) %>%
  ungroup() %>%
  group_by(site, precip, stage, trait, id) %>%
  summarise(cm = mean(val, na.rm = T)) %>%
  group_by(site, precip, Stage = stage, trait) %>%
  filter(length(unique(id)) == 2) %>%
  summarise(cm_diff = cm[id == 'Transient'] - cm[id == 'Persistent']) %>%
  filter(!is.na(cm_diff))
...
mods <- j %>%
  group_by(trait, Stage) %>%
  do(mod = lm(cm_diff ~ precip, data = .)) %>%
  mutate(pval = summary(mod)$coefficients[8])

j <- j %>%
  left_join(mods[, c('trait','Stage','pval')], by = c("trait", "Stage"))

ggplot(j, aes(x = precip, y = cm_diff, color = Stage, fill = Stage)) +
    geom_hline(yintercept = 0, lty = 3, color = 'dark grey') +
    geom_point() + 
    stat_smooth(method = 'lm', se = FALSE, data = filter(j, pval < 0.05)) +
    facet_wrap(~trait, scales = 'free', ncol = 2) +
    th +
    labs(x = 'Mean annual precipitation (mm/yr)', y = 'Transient community mean - Persistent community mean')

```

**Figure S5.** Community mean trait values of locally transient species relative to locally persistent species, grouped by site and life stage and plotted by mean annual precipitation. Community means are calculated as the average trait value of all species present in the community. Values above the grey dotted line (y = 0) denote sites where the trait values of transient individuals were greater on average than the trait values of persistent individuals, and vice versa. Lines are shown only when linear regression is significant (p < 0.05).

\pagebreak

```{r density and richness table, cache = FALSE}
...
```{r linear model stats table all species, cache = FALSE}
...
```{r linear model stats table transients only, cache = FALSE}
...
```{r species site correlations, cache = FALSE}
...
```{r seedling establishment model table, cache = FALSE}
...
j <- mod_coefs %>%
  ...
  transmute(
    ...
    `Null model` = estNull, 
    `Local climate` = estClim,
    `Local climate + Local sp. status (P/T)` = estS,
    `Local climate + Local sp. status (P/ST/C/Wa/U)` = estT,
    `Local climate + Local sp. status (P/SP/D/We/U)` = estP)

AICs <- c('$\\Delta$AIC', NA, sprintf("%.2f", c(0, AIC(estClim) - AIC(estNull), 
                                          AIC(estS) - AIC(estNull),
                                          AIC(estT) - AIC(estNull), 
                                          AIC(estP) - AIC(estNull))))
...
kable(j, format = 'latex', escape = F, booktabs = T, linesep = "", align = c('l','r','r','r','r','r','r')) %>%
  ...
  group_rows("Origin-based predictors", 8, 14) %>%
  footnote(threeparttable = TRUE, general = 'Comparison of z-scores from different GLM models (columns) predicting numbers of established seedlings by species and site. Abbreviations in column headers refer to the categories used in the model term denoting local species status, potentially including the putative climate origins of each transient species at each site: P = persistent, T = transient, ST = similar temperature (i.e., a transient from a site with a similar temperature), SP = similar precipitation, C = cooler, Wa = warmer, D = drier, We = wetter, or U = unknown climate. Data include all recorded seedlings that could be identified to species. Asterisks denote significance (*: p < 0.05, **: p < 0.001). Dashes indicate predictors that were not included in a given model. Including model terms for transient/persistent species status and then putative temperature and precipitation origins did not improve model performance, as reflected in the increase of AIC values relative to the null model based only on local numbers of emerged seedlings.', general_title = "Table S6.") %>%
  landscape()
...
```{r species emergence model table, cache = FALSE}
...
j <- mod_coefs %>%
  ...
  transmute(
    ` ` = as.character(Variable),
    N = c(rep(sum(germ_n[c(1,2,4,5,8)]), 3), rep(sum(germ_n[c(2,4,5,8)]), 3), germ_n[c(2,4,5,3,6,7,8)]),
    `Null model` = germbinNull, 
    `Local climate` = germbinClim,
    `Local climate + Local sp. status (P/T)` = germbinS,
    `Local climate + Local sp. status (P/ST/C/Wa/U)` = germbinT,
    `Local climate + Local sp. status (P/SP/D/We/U)` = germbinP)

AICs <- c('$\\Delta$AIC', NA, sprintf("%.2f", c(0, AIC(germbinClim) - AIC(germbinNull), 
                                                   AIC(germbinT) - AIC(germbinNull), 
                                                   AIC(germbinS) - AIC(germbinNull),
                                                   AIC(germbinP) - AIC(germbinNull))))
...
```{r species establishment model table, cache = FALSE}
...
j <- mod_coefs %>%
  ...
  transmute(
    ` ` = as.character(Variable),
    N = c(rep(sum(est_n[c(1,2,4,5,8)]), 3), rep(sum(est_n[c(2,4,5,8)]), 3), est_n[c(2,4,5,3,6,7,8)]),
    `Null model` = estbinNull, 
    `Local climate` = estbinClim,
    `Local climate + Local sp. status (P/T)` = estbinS,
    `Local climate + Local sp. status (P/ST/C/Wa/U)` = estbinT,
    `Local climate + Local sp. status (P/SP/D/We/U)` = estbinP)

AICs <- c('$\\Delta$AIC', NA, sprintf("%.2f", c(0, AIC(estbinClim) - AIC(estbinNull), 
                                          AIC(estbinS) - AIC(estbinNull), 
                                          AIC(estbinT) - AIC(estbinNull), 
                                          AIC(estbinP) - AIC(estbinNull))))
...
"How many adults were never seen as seeds outside of thier local sites"
sum(!unique(x$sp[x$stage == 'mature' & x$id == 'Persistent']) %in% xS$sp[xS$stage == 'seed' & xS$id == 'Transient'])

"How many of the adult species had no seeds observed anywhere"
